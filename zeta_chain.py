...
class ZetaChain:
    def __init__(self, key: str, thread: int, proxy=None):
        ...
        self.contract_for_encoding = self.web3_utils.w3.eth.contract(
            address=self.web3_utils.w3.to_checksum_address("0x8Afb66B7ffA1936ec5914c7089D50542520208b8"),
            abi=abi.encoding_contract_abi,)
        self.main_contract = self.web3_utils.w3.eth.contract(
            address=self.web3_utils.w3.to_checksum_address("0x34bc1b87f60e0a30c0e24FD7Abada70436c71406"),
            abi=abi.multicall_abi,)
    ...
    async def check_completed_task(self, task):
        uri = f'https://xp.cl04.zetachain.com/v1/get-user-has-xp-to-refresh?address={self.web3_utils.acct.address}'
        resp = await self.session.get(uri, proxy=self.proxy)
        resp_json = await resp.json()

        res = (resp_json.get('xpRefreshTrackingByTask').get(task).get('hasXpToRefresh') is False and
               resp_json.get('xpRefreshTrackingByTask').get(task).get('hasAlreadyEarned') is False)

        return res

    async def check_enroll(self):
        json_data = {'address': self.web3_utils.acct.address}
        uri = "https://xp.cl04.zetachain.com/v1/enroll-in-zeta-xp"
        resp = await self.session.post(uri, json=json_data, proxy=self.proxy)
        a = await resp.json()
